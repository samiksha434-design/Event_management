    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const totalRevenue = events.reduce((total, event) => {
          ...
          return total + (fees * participants);
        }, 0);

        // Calculate college statistics
        const collegeData = {};
        events.forEach(event => {
          const collegeName = event.college || 'Unknown';
          ...
          collegeData[collegeName].events += 1;
          const participants = event.participants ? event.participants.length : 0;
          collegeData[collegeName].participants += participants;
          collegeData[collegeName].revenue += (event.fees || 0) * participants;
        });
        ...
        setCollegeStats(collegeData);
        
        // Fetch analytics
        try {
          const analyticsData = await eventService.getEventAnalytics();
    ...
    <div className="pb-12">
      <div className="bg-white rounded-xl shadow-md overflow-hidden mb-8">
        ...
        <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6">
                  ...
                  <div className="bg-white p-4 rounded-lg shadow text-center">
                    ...
                    <div className="flex items-center justify-center mt-2">
                      ...
                      <p className="text-gray-500 text-sm">Events today</p>
                    </div>
                  </div>
                  <div className="bg-white p-4 rounded-lg shadow text-center">
                  ...
                  <div className="bg-white p-4 rounded-lg shadow text-center">
                    ...
                    <div className="flex items-center justify-center mt-2">
                      ...
                      <p className="text-gray-500 text-sm">Event fees collected</p>
                    </div>
                  </div>
                  <div className="bg-white p-4 rounded-lg shadow text-center">
                    ...
                    <div className="flex items-center justify-center mt-2">
                      <svg className="w-4 h-4 text-teal-500 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="mt-8">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">College-wise Statistics</h2>
            <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
              {Object.keys(collegeStats).length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {Object.entries(collegeStats).map(([college, data]) => (
                        <tr key={college} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{college}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{data.events}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{data.participants}</td>
                ...
                <p className="text-gray-500 text-center py-4">No college data available yet.</p>
              )}
            </div>
          </div>

          {/* Category Popularity Section */}
          {analytics?.categoryPopularity && (
            <div className="mt-8">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">Event Category Popularity</h2>
              <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
                  {Object.entries(analytics.categoryPopularity).map(([category, count]) => (
                    <div key={category} className="bg-white p-4 rounded-lg shadow text-center">
                      <h3 className="text-gray-500 text-xs font-medium uppercase">{category}</h3>
